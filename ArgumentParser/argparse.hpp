...
template <typename T> struct is_standard_type
{
    const static bool value = false;
};

template <> struct is_standard_type<float>
{
    const static bool value = true;
};
template <> struct is_standard_type<double>
{
    const static bool value = true;
};
template <> struct is_standard_type<long double>
{
    const static bool value = true;
};
template <> struct is_standard_type<short>
{
    const static bool value = true;
};
template <> struct is_standard_type<int>
{
    const static bool value = true;
};
template <> struct is_standard_type<long>
{
    const static bool value = true;
};
template <> struct is_standard_type<unsigned short>
{
    const static bool value = true;
};
template <> struct is_standard_type<unsigned int>
{
    const static bool value = true;
};
template <> struct is_standard_type<unsigned long>
{
    const static bool value = true;
};
template <> struct is_standard_type<char>
{
    const static bool value = true;
};
template <> struct is_standard_type<signed char>
{
    const static bool value = true;
};
template <> struct is_standard_type<unsigned char>
{
    const static bool value = true;
};
template <> struct is_standard_type<std::string>
{
    const static bool value = true;
};
...
public:
    ...
    void addArgument(const String& name, bool optional = true, char nargs = '*')
    {
        if (name.size() > 2)
        {
    ...
    void addArgument(const String& short_name, const String& name, bool optional = true, char nargs = '*')
    ...
    void parse(size_t argc, const char** argv)
    {
        ...
        {
            std::string ar = std::string(argv[i]);
            size_t pos = ar.find("=");
            if(pos && pos != std::string::npos)
            {
                sv.push_back(ar.substr(0, pos));
                if(ar.length() - pos > 1)
                {
                    sv.push_back(ar.substr(pos + 1, ar.length()));
                }
            }
            else
            {
    ...
    void parse(const StringVector& argv)
    {
        ...
        bool IgnoreInvalidKeyValue = false;
                ...
                in < argv.end() - nfinal; ++in)
        {
            ...
            if (index_.count(el) == 0)
            {
                ...
                else if(el.find("-") != std::string::npos
                        || el.find("--") != std::string::npos
                        || IgnoreInvalidKeyValue)
                {
                    IgnoreInvalidKeyValue = true;
                }
                else
                {
